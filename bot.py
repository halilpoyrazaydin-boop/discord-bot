...
async def rutbe_komutu(interaction: discord.Interaction, kullanici_adi: str, sebep: str, islem: str, filtre: str = ""):
    ...
    # Filtre uygula
    if filtre:
        secenekler = [r for r in secenekler if r["name"].lower().startswith(filtre.lower())]
    if not secenekler:
        await interaction.followup.send(f"'{filtre}' ile baslayan uygun rutbe bulunamadi!", ephemeral=True)
        return
...
@bot.tree.command(name="rutbever", description="Roblox grubunda kisiye rutbe ver")
@app_commands.describe(kullanici_adi="Roblox kullanici adi", sebep="Sebep", filtre="Rutbe adinin bas harfi (opsiyonel, cok rutbe varsa kullan)")
@app_commands.checks.has_permissions(manage_roles=True)
async def rutbe_ver(interaction: discord.Interaction, kullanici_adi: str, sebep: str = "Belirtilmedi", filtre: str = ""):
    await rutbe_komutu(interaction, kullanici_adi, sebep, "ver", filtre)

@bot.tree.command(name="rutbe-terfi", description="Roblox grubunda kisiye terfi ver")
@app_commands.describe(kullanici_adi="Roblox kullanici adi", sebep="Sebep", filtre="Rutbe adinin bas harfi (opsiyonel)")
@app_commands.checks.has_permissions(manage_roles=True)
async def rutbe_terfi(interaction: discord.Interaction, kullanici_adi: str, sebep: str = "Belirtilmedi", filtre: str = ""):
    await rutbe_komutu(interaction, kullanici_adi, sebep, "terfi", filtre)

@bot.tree.command(name="rutbe-tenzil", description="Roblox grubunda kisiyi tenzil et")
@app_commands.describe(kullanici_adi="Roblox kullanici adi", sebep="Sebep", filtre="Rutbe adinin bas harfi (opsiyonel)")
@app_commands.checks.has_permissions(manage_roles=True)
async def rutbe_tenzil(interaction: discord.Interaction, kullanici_adi: str, sebep: str = "Belirtilmedi", filtre: str = ""):
    await rutbe_komutu(interaction, kullanici_adi, sebep, "tenzil", filtre)
